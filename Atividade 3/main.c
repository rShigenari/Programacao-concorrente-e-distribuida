 ...
 */
{
  ...
  t = MULTIPLIER * (seedVet[i] % Q) - R * (seedVet[i] / Q);
  if (t > 0)
   seedVet[i] = t;
  else
    seedVet[i] = t + MODULUS;
  return ((double) seedVet[i] / MODULUS);
}
...
int main()
{
  ...
  seedVet = (long*) malloc(N*sizeof(long));
...
#pragma omp parallel for private(rmin, i, xi, yi, fx, fy) num_threads(8)
  for (i=0; i<npart; i++) {
    rmin = 100.0;
    xi   = myparticles[i].x;
    yi   = myparticles[i].y;
    fx   = 0.0;
    fy   = 0.0;
#pragma omp parallel for reduction(-:fx,fy) private(j, rx, ry, mj, r) num_threads(8)
    for (j=0; j<npart; j++) {
      rx = xi - others[j].x;
      ry = yi - others[j].y;
      mj = others[j].mass;
